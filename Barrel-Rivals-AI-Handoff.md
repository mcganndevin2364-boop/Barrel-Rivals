...
Latest art-only revision: [terrain checkpoint](Docs/Art/Reins-Landscape-Checkpoint.md). Credited USGS elevation data replaces the coarse procedural terrain; an original world-mapped rock/soil shader, restrained mineral/sage colors and reduced haze improve the backdrop. [Pinned acquisition and reproduction](Docs/Art/Elevation-Source.md) require no live download to regenerate. Fresh tests pass 137/137 Editor and 33/33 Play Mode, including a terrain pixel/pass regression. New evidence uses `Landscape-*`; all 386 prior evidence records remain unchanged. Terrain/pines total 28,768 triangles and three opaque material slots. Horse/stable/gear functionality and Core/contracts/verifier are unchanged; no new HTTP run, native build or device acceptance. The [mane crown checkpoint](Docs/Art/Reins-Mane-Flow-Checkpoint.md) remains the current character revision. The game still falls below the supplied photographic references.
