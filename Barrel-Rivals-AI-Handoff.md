...
Latest art-only revision: [fiber shading checkpoint](Docs/Art/Reins-Fiber-Shading-Checkpoint.md). Racing, MyStable and the rendered portrait share a new original strand-direction material with restrained highlights and edge lighting. Groom/atlas/FBX/animation bytes are unchanged. The existing floodlights also retain their authored glow through material revalidation. Fresh tests pass 138/138 Editor and 35/35 Play Mode, including tangent-frame persistence, actual directional highlights, coverage and saved-material scene checks. New evidence uses `FiberShading-*`; all 427 prior evidence records remain unchanged. Core/contracts/verifier are unchanged; no new HTTP run, native build or device acceptance. The [terrain checkpoint](Docs/Art/Reins-Landscape-Checkpoint.md) and [mane crown geometry checkpoint](Docs/Art/Reins-Mane-Flow-Checkpoint.md) remain relevant source history. The game still falls below the supplied photographic references.
