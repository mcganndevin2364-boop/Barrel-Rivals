...
2. Preserve the verified 0.5 iPhone artifact and build a fresh Android artifact using current source. Export iOS outside cloud-coordinated storage per [recorded guidance](README-iPhone.md). Inspect current verifier flags before invoking artifact scripts; earlier 0.4 records are historical. Keep raw logs, signing credentials, caches and binaries out of Git.
...
Fresh 0.5 iOS export, native compilation, signing and IPA integrity are verified; installation/play/performance remain pending. The last verified installed phone app remains 0.4.0/build 4. See [the mobile checkpoint](Docs/Build/Reins-0.5-iPhone-Checkpoint.md) and [phone review](Docs/Build/Reins-0.5-Phone-Review.md). Native export/build files and private signing state remain outside Git. No Android 0.5 artifact or phone performance is claimed.
<!-- REINS05_MOBILE_END -->
