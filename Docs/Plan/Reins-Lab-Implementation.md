...
- iPhone **0.4.0/build 4** compiled, signed and passed signature/profile/ARM64/IPA checks; USB installation and the targeted installed-version query passed. Manual launch/control/performance acceptance remains pending. The Android artifact is still building after enabling the installed built-in `com.unity.modules.androidjni` 1.0.0 dependency required by the haptic bridge; its first compile identified that missing module. The Reins simulation and input source are unchanged. Earlier 0.2.0 user confirmation is not reused for either newer build.
