    ...
    public sealed class ReinsMotionTests
    {
        [UnityTest] public IEnumerator RenderedGaitDrivesCameraAndHandsWithoutMovingTheRaceRoot()
        {
            ...
            try {
                ...
                animator.Play(0,0,.25f);animator.Update(0);tack.RenderImmediate();rig.RenderImmediate();
                Assert.That(source.GaitPhaseRadians,Is.EqualTo(Mathf.PI*.5f).Within(.002f));
                ...
                Vector3 cameraHigh=camera.transform.position,handHigh=left.localPosition;
                AssertReinEndpoint(rein,0,grip.position);
                AssertReinEndpoint(rein,ReinsRiderTackPresentation.Segments*8,bit.position);
                animator.Play(0,0,.75f);animator.Update(0);tack.RenderImmediate();rig.RenderImmediate();
                Assert.That(source.GaitPhaseRadians,Is.EqualTo(Mathf.PI*1.5f).Within(.002f));
                ...
                Assert.That(cameraHigh.y-camera.transform.position.y,Is.EqualTo(.024f).Within(.001f));
                Assert.That(handHigh.y-left.localPosition.y,Is.EqualTo(.016f).Within(.001f));
                AssertReinEndpoint(rein,0,grip.position);
                rig.SetReducedMotion(true);Vector3 reducedPosition=camera.transform.position;
                animator.Play(0,0,.25f);animator.Update(0);tack.RenderImmediate();rig.RenderImmediate();reducedPosition=camera.transform.position;
                animator.Play(0,0,.75f);animator.Update(0);tack.RenderImmediate();rig.RenderImmediate();
                ...
                foreach(float speed in new[]{1.5f,8f,14f,30f}) {
                    ...
                    Assert.That(animator.speed,Is.InRange(1f,1.35f));
                    Assert.That(animator.speed,Is.EqualTo(speed<=8?1:1.35f).Within(.0001f));
                ...
                animator.Play(0,0,.25f);animator.Update(0);rig.RenderForCapture(.04f);
                Assert.That(camera.transform.position.y-idleHeight,Is.InRange(.003f,.011f));
