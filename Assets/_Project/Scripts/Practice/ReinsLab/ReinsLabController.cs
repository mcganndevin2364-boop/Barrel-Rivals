...
namespace BarrelRivals.Practice
{
    ...
    public sealed class ReinsLabController : MonoBehaviour
    {
        ...
        private bool sound=true,bodycam=true,ghostEnabled=true,recordedFinish;
        ...
        private void Awake()
        {
            ...
            // Clone while the source is in its serialized neutral pose, before the first simulation step.
            ghostHorse=Instantiate(horse,horse.parent);ghostHorse.name="Own best — Reins recording";
            ...
            { var materials=new Material[renderer.sharedMaterials.Length];for(int i=0;i<materials.Length;i++)materials[i]=ghostMaterial;renderer.sharedMaterials=materials;renderer.shadowCastingMode=UnityEngine.Rendering.ShadowCastingMode.Off;renderer.receiveShadows=false; }
        ...
        internal void RefreshPresentation(){lastFrame=Time.realtimeSinceStartupAsDouble;Present();}
        internal void Step(ReinsInput input)
        {
            var before=run.Phase;run.Step(input);if(recorded.Count<7500)recorded.Add(input);
            if(ghostRun!=null && ghostFrame<savedBest.frames.Length)ghostRun.Step(savedBest.frames[ghostFrame++].Input());
        ...
        private void Present()
        {
            ...
            surfaceText.text="DIRT: "+SurfaceName(surface);soundText.text=sound?"SOUND ON":"SOUND OFF";cameraText.text=bodycam?"BODYCAM":"CHASE CAM";
            ...
            bool overview=ready || run.Phase==ReinsPhase.Preview;
            Vector3 target=overview?new Vector3(0,47,-24):horse.TransformPoint(bodycam?new Vector3(0,2.7f,-.35f):new Vector3(0,5,-8));
            Quaternion rotation=overview?Quaternion.LookRotation(new Vector3(0,0,24)-target):horse.rotation*Quaternion.Euler(bodycam?7:17,0,0);
            rideCamera.transform.SetPositionAndRotation(target,rotation);rideCamera.fieldOfView=65;
            map.Show(run);
            if(ghostHorse)
            {
                bool show=ghostEnabled && ghostRun!=null && !overview && !terminal && !Terminal(ghostRun.Phase);
                if(show){var position=new Vector3((float)ghostRun.X,0,(float)ghostRun.Z);show=(position-horse.position).sqrMagnitude>5;ghostHorse.SetPositionAndRotation(position,Quaternion.Euler(0,(float)(ghostRun.HeadingRadians*180/Math.PI),0));}
        ...
        private void ToggleCamera(){bodycam=!bodycam;Present();}
        ...
        private void CancelRun(){if(run==null)return;if(run.Phase!=ReinsPhase.Ready && !Terminal(run.Phase))run.Cancel();ClearInput();accumulator=0;beatVoice?.Stop();gradeVoice?.Stop();if(ghostHorse)ghostHorse.gameObject.SetActive(false);Present();}
