...
start = 1
for view in ('side', 'quarter', 'rider'):
    ...
    strip.frame_final_duration = 4 * frames
    text = seq.strips.new_effect('Study label ' + view, type='TEXT', channel=2, frame_start=start, frame_end=start + 4 * frames)
    text.text = 'HORSE MOTION STUDY  |  ' + view.upper() + ' VIEW\nOffline render - untextured - not final gameplay'
    text.font_size = 22
...
(folder / 'video-spec.json').write_text(json.dumps({'frames': start - 1, 'fps': 30, 'width': 960, 'height': 720, 'views': ['side', 'quarter', 'rider'], 'groundReferenceSpeedMps': spec['authoredSpeedMps'], 'scope': 'Offline diagnostic frames, not a Unity or device capture.'}, indent=2) + '\n')
