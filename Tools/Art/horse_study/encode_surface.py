...
for view in ('quarter', 'rider'):
    ...
    label = seq.strips.new_effect('Surface study ' + view, type='TEXT', channel=2, frame_start=start, frame_end=start + 112)
    label.text = 'HORSE FACE / COAT STUDY  |  ' + view.upper() + '\nOffline 3D render - not gameplay or final art'
    label.font_size = 19
...
s.render.filepath = str(folder / 'HeroHorse-SurfaceStudy.mp4')
bpy.ops.render.render(animation=True)
(folder / 'video-spec.json').write_text(json.dumps({'frames': 224, 'fps': 30, 'width': 800, 'height': 600, 'views': ['quarter', 'rider'], 'scope': 'Offline material review, static floor, four repeated cycles per view. Not measured gameplay performance.'}, indent=2) + '\n')
print('SURFACE_VIDEO_ENCODED')
