...
Offline rotation IK; no constraints, bone scale, runtime solver or root motion.
...
for i in range(frames * 4 + 1):
    ...
    frame = 1 + i / 4
    reset()
    arm.pose.bones['Root'].location = arm.data.bones['Root'].matrix_local.to_3x3().inverted() @ Vector((0, 0, -0.12 + 0.006 * math.cos(phase * math.tau * 2)))
    angle('NeckLower', 0.015 * math.sin(phase * math.tau))
    angle('NeckUpper', -0.012 * math.sin(phase * math.tau + 0.3))
    bpy.context.view_layer.update()
    targets = {}
    for limb in limbs:
    ...
    for limb in limbs:
        ...
        fetlock = p - limb['hoofOffset'] - limb['pasternVector']
        maxreach = max(maxreach, solve(limb, fetlock))
        for name in (limb['pastern'], limb['hoof']):
            rotation(arm.pose.bones[name], arm.data.bones[name].matrix_local.to_quaternion())
    for pb in arm.pose.bones:
        if pb.name != 'Root':
    ...
    for limb in limbs:
        ...
        (p, contact, cycle) = targets[limb['label']]
        offsets_z = [coords[n].z - rest[n].z for n in limb['rigidIndices']]
        row['hooves'][limb['label']] = {'center': list(center), 'lowest': min((v.z for v in rigid)), 'contact': contact, 'cycle': cycle, 'solePlaneErrorM': max(offsets_z) - min(offsets_z), 'targetErrorM': (center - p).length}
...
scene.frame_set(1)
report = {'scope': 'Offline untextured 34-bone walk study. Not natural full-gait, Unity or phone acceptance.', 'sourceRigSha256': hashlib.sha256((folder / 'HeroHorse-RigStudy.fbx').read_bytes()).hexdigest(), 'fps': 30, 'frames': frames, 'durationSeconds': duration, 'authoredSpeedMps': speed, 'dutyFraction': duty, 'swingClearanceM': clearance, 'samples': len(samples), 'objectRootMotion': False, 'boneScaleAnimation': False, 'hoofOrientation': 'Neutral world orientation held by independent hoof and pastern joints; no roll-over yet.', 'maximumReachClampingM': maxreach, 'maximumNonRootBoneTranslationM': maxlocaltranslation, 'minimumBodyHeightM': minheight, 'maximumEdgeStretch': maxstrain, 'feet': {}}
for limb in limbs:
    label = limb['label']
    points = [s['hooves'][label] for s in samples]
    stance = [p for p in points if p['contact']]
    positions = [p['center'][1] + speed * duration * p['cycle'] for p in stance]
    report['feet'][label] = {'stanceResidualTravelM': max(positions) - min(positions), 'minimumStanceHeightM': min((p['lowest'] for p in stance)), 'maximumStanceHeightM': max((p['lowest'] for p in stance)), 'maximumSolePlaneErrorM': max((p['solePlaneErrorM'] for p in points)), 'maximumTargetErrorM': max((p['targetErrorM'] for p in points)), 'peakSwingHeightM': max((p['lowest'] for p in points))}
